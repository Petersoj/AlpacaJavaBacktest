 ...
 * submit orders to {@link BacktestBroker} accordingly. Simply create a new class that extends this one and use
 * {@link AlpacaJavaBacktest#backtest()} to execute a backtest.
...
public abstract class AlgorithmTrader {
     ...
     */
    public abstract void onTradeUpdate(String ticker, io.github.mainstringargs.polygon.domain.historic.trades.Tick trade);
     ...
     */
    public abstract void onQuoteUpdate(String ticker, io.github.mainstringargs.polygon.domain.historic.quotes.Tick quote);

    /**
     * This method is executed when an order placed via the BacktestBroker is filled.
     * Note that an order may be filled completely or partially filled. Use {@link Order#getFilledQty()} to get how much
     * of the order was actually filled.
     * It is a good idea to implement some sort of order tracking mechanism in a trading algorithm so that, in the event
     * that an order is not completely filled, your algorithm can adjust/cancel the order accordingly.
     *
